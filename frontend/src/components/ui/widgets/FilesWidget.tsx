...
import { getPcap, deletePcap, getPcapById } from "../../../api";
...
export function FilesWidget() {
    ...
    const getPcapByIdMutation = useMutation({
        mutationFn: getPcapById,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["pcaps"] });
          },
    })

    function onAnalysisClick(id : undefined|string){
        if (id != undefined){
            getPcapByIdMutation.mutate(id)
        }
    }
                ...
                <div className="space-y-4">
                    <div className="bg-[#2d3748] w-fit p-4 rounded-lg">
                        ...
                        <div className="space-x-2 mt-2">
                            <button onClick={()=> onAnalysisClick(selectedFile)}  className="px-3 py-1 bg-[#4a5568] rounded hover:bg-[#2d3748] transition-colors">
                                Analysis
                            </button>
                            <button onClick={()=> onDeleteClick(selectedFile)} className="px-3 py-1 bg-[#e53e3e] rounded hover:bg-[#c53030] transition-colors">
                                Delete
                            </button>
                        </div>
                    </div>
